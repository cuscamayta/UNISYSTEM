...
function showModalResource(career) {
    $("#btn-save-resource").unbind().click(function () {
        saveResource(career);
    });
    ...
    validateControlsModalResource();
    $("input, select").on('change keyup paste', function () {
        validateControlsModalResource();
    });
}

function validateControlsModalResource() {
    if ($('#resource-autor').val().length > 0 && $('#resource-link').val().length > 0
    && $('#resource-title').val().length > 0 && $('#type-resource').val().length > 0) {
        $("#btn-save-resource").prop('disabled', false);
    } else {
        $("#btn-save-resource").prop('disabled', true);
    }
}

function afterSaveResources(response) {
    if (response.IsSuccess) {
        toastr.success("Recurso cargado correctamente");
...
function saveResource(career) {
    if (validateMemberCode() === $("#resource-code").val()) {
        var newResource = {
            Author: $('#resource-autor').val(),
            Description: $('#resource-description').val(),
            ResourceLink: $('#resource-link').val(),
            Title: $('#resource-title').val(),
            NameTypeResource: $('#type-resource').val(),
            NameCareer: career
        };
        ajaxRequest('/Resources/SaveResource', afterSaveResources, newResource);
    } else {
        toastr.warning("Codigo de miembro no valido");
    }
}
...
function validateMemberCode() {
    ...
    return "UCB-" + year + monthNames[monthIndex] + (day < 9 ? "0" + day : day);
}

function showModalNew() {
    $("#btn-save-new").unbind().click(function () {
        saveNew();
    });
    $("#modalNew").modal("show");
    $('#new-title').val("");
    $('#new-content').val("");
    $('#new-code').val("");
    validateControlsModalNew();
    $("input, select").on('change keyup paste', function () {
        validateControlsModalNew();
    });
}

function afterSaveNews(response) {
    if (response.IsSuccess) {
        toastr.success("Noticia cargado correctamente");
        $("#modalNew").modal("hide");
    } else {
        toastr.error("Error al cargar");
    }
...
function saveNew() {
    if (validateMemberCodeAdm() === $("#new-code").val()) {
        var news = {
            Title: $('#new-title').val(),
            Resume: $('#new-content').val(),
        };
        ajaxRequest('/News/SaveNews', afterSaveNews, news);
    } else {
        toastr.warning("Codigo de miembro administrador no valido");
    }
}

function validateControlsModalNew() {
    if ($('#new-title').val().length > 0 && $('#new-content').val().length > 0) {
        $("#btn-save-new").prop('disabled', false);
    } else {
        $("#btn-save-new").prop('disabled', true);
    }
}

function validateMemberCodeAdm() {
    var monthNames = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"];
    ...
    return "UCB-ADM-" + year + monthNames[monthIndex] + (day < 9 ? "0" + day : day);
}
